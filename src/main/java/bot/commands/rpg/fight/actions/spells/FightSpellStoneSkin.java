...
public class FightSpellStoneSkin implements FightActionHandler {

	private final Fluffer10kFun fluffer10kFun;

	public FightSpellStoneSkin(final Fluffer10kFun fluffer10kFun) {
		this.fluffer10kFun = fluffer10kFun;
	}

	@Override
	public void handle(final FightTempData data, final RPGFightAction action) {
		final boolean spellCast = fluffer10kFun.fightActionUtils.castSpell(data, STONE_SKIN, 5);
		if (!spellCast) {
			return;
		}
		...
		data.activeFighter.statuses.addStatus(new FighterStatusData(FighterStatus.STONE_SKIN).duration(duration));
		data.fight.addTurnDescription(action.description(data.activeFighter.name));

		if (player != null) {
			player.lastSpellUsed = STONE_SKIN;
		}
	}
}
