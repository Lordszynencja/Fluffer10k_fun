...
public class FightCurse implements FightActionHandler {
	private final Targetting defaultTargetting;

	private final Fluffer10kFun fluffer10kFun;

	public FightCurse(final Fluffer10kFun fluffer10kFun) {
		this.fluffer10kFun = fluffer10kFun;

		final TargetCheck validTarget = TargetCheck.ENEMY.alive().without(fluffer10kFun, FighterClass.MECHANICAL);

		defaultTargetting = new Targetting(validTarget.without(FighterStatus.CURSED))//
				.orElse(validTarget);
	}
	...
	public void handle(final FightTempData data, final RPGFightAction action) {
		...
		data.target.statuses.addStatus(new FighterStatusData(FighterStatus.CURSED).duration(duration));
